...
public class and_there_is_one_document_in_the_query : Specification
{
    ...
    void Establish()
    {
        _set = new(QueryContextBuilder.New()
            ...
            .WithPageSize(4)
            .Build());
        _queryDocuments = [new(Guid.NewGuid(), 49)];
    ...
    [Fact] void should_have_items_in_correct_order() => Assert.Equal([_firstItem, _secondItem, _fourthItem, _queryDocuments.First()], _set);
}