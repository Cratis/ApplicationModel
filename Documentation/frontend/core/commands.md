...
**It's recommended to configure this globally** through the `<Arc />` component's `microservice` property. This automatically applies to all commands and queries in your application. See the [Arc Configuration](../react/application-model.md#microservice-support) for details.
...
**However, it's recommended to configure this globally** using the `<Arc />` component's `apiBasePath` property. This ensures consistency across all commands and queries. See the [Arc Configuration](../react/application-model.md#configuration-options) for details.
